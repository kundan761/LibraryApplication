...
exports.createBook = auth, async (req, res) => {
  try {
...
exports.updateBook = auth, async (req, res) => {
...
exports.deleteBook = auth, async (req, res) => {
  const { bookId } = req.body;
  try {
    const deletedBook = await Book.findByIdAndDelete({ _id: bookId });
    res.status(200).json({ msg: "Deleted Successfully", deletedBook });
  } catch (error) {
    res.status(500).json({ msg: "Something went wrong" });
  }
};
